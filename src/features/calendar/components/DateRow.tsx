...
import type { Habit, Check } from "@/lib/types/db";
...
export function DateRow({ date, habits, checks, onToggle, onEditNote }: DateRowProps) {
    ...
    return (
        <div className={cn(
            ...
            isToday ? "bg-accent/10 hover:bg-accent/20" : "border-b border-border/40"
        )}>
            {/* Date Column */}
            <div className={cn(
                ...
                <span className="text-[10px] uppercase tracking-wider leading-none mt-1">{dayOfWeek}</span>
            </div>
            ...
            <div className="flex flex-1 gap-2 overflow-x-hidden px-2">
                {habits.map(habit => {
                    ...
                    const check = checks.find(c => c.habitId === habit.id && c.date === targetDateStr);
